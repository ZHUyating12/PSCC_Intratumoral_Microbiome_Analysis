...
fungi_bacteria<-readRDS("/home/zyt/16s_JiangXi/model_R/Tis_T1_model/fungi_bacteria/data_model.rds")
...
train_roc <- roc(response = train_data$group, predictor = train_prob[,"T1"])
auc(train_roc)
...
test_roc <- roc(response = test_data$group, predictor = test_prob[,"T1"])
auc(train_roc)
auc(test_roc)

##plot 
plot.roc(
  train_roc$response,train_roc$predictor,col = "#A31621",
  percent = TRUE,lwd = 2,print.auc.cex = 1,
  print.auc = TRUE,print.auc.pattern = "Train(Fungi + Bac): %.1f%%",
