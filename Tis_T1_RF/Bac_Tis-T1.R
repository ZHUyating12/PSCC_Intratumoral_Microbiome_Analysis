...
bacteria<-readRDS("/home/zyt/16s_JiangXi/model_R/Tis_T1_model/bacteria/data_model.rds")
set.seed(123)
#Bac
data_model<-bacteria
...
bactrain_roc <- roc(response = train_data$group, predictor = train_prob[,"T1"])
auc(bactrain_roc)
...
bactest_roc <- roc(response = test_data$group, predictor = test_prob[,"T1"])
auc(bactrain_roc)
auc(bactest_roc)
