...
fungi<-readRDS("/home/zyt/16s_JiangXi/model_R/LN_positive_negative/data/fungi.rds")
...
fugitrain_roc <- roc(response = train_data$group, predictor = train_prob[,"postive"])
auc(fugitrain_roc)
...
fugitest_roc <- roc(response = test_data$group, predictor = test_prob[,"postive"])
auc(fugitest_roc)
auc(fugitrain_roc)

plot.roc(
  fugitrain_roc$response,fugitrain_roc$predictor,col = "#FF5F5F",
  percent = TRUE,lwd = 2,print.auc.cex = 1,print.auc = TRUE,
  print.auc.pattern = "Train(Fungi): %.1f%%",print.auc.y = 62,print.auc.x = 60,add=T)
plot.roc(
  fugitest_roc$response,fugitest_roc$predictor,col = "#fb8500", 
  percent = TRUE,lwd = 2,print.auc.cex = 1,print.auc = TRUE,
