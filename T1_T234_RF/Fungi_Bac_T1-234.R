...
fungi_bacteria<-readRDS("/home/zyt/16s_JiangXi/model_R/T1_T234/data/fungi_bacteria.rds")
#fungi_bacteria
set.seed(123)
...
train_roc <- roc(response = train_data$group, predictor = train_prob[,"Early"])
auc(train_roc)
...
test_roc <- roc(response = test_data$group, predictor = test_prob[,"Early"])
auc(train_roc)
auc(test_roc)

##plot 
plot.roc(
  train_roc$response,train_roc$predictor,
  col = "#A31621",percent = TRUE,lwd = 2,print.auc.cex = 1,print.auc = TRUE,print.auc.pattern = "Train(Fungi + Bac): %.1f%%",
  print.auc.y = 70,print.auc.x = 60)
plot.roc(
  test_roc$response,test_roc$predictor,
  col = "#8134af",percent = TRUE,lwd = 2,
  print.auc.cex = 1,print.auc = TRUE,print.auc.pattern = "Test(Fungi + Bac): %.1f%%",
