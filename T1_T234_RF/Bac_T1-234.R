...
bacteria<-readRDS("/home/zyt/16s_JiangXi/model_R/T1_T234/data/bacteria.rds")
#Bac
data_model<-bacteria
...
bactrain_roc <- roc(response = train_data$group, predictor = train_prob[,"Early"])
auc(bactrain_roc)
...
bactest_roc <- roc(response = test_data$group, predictor = test_prob[,"Early"])
auc(bactrain_roc)
auc(bactest_roc)


plot.roc(
  bactrain_roc$response,bactrain_roc$predictor,
  col = "#E06681",percent = TRUE,lwd = 2,
  print.auc.cex = 1,print.auc = TRUE,print.auc.pattern = "Train(Bac): %.1f%%",
  print.auc.y = 54,print.auc.x = 60,add=T)

plot.roc(
  bactest_roc$response,bactest_roc$predictor,col = "#2694ab",
  percent = TRUE,lwd = 2,print.auc.cex = 1,
  print.auc = TRUE,print.auc.pattern = "Test(Bac): %.1f%%",
  print.auc.y = 30,print.auc.x = 60,add=T)
